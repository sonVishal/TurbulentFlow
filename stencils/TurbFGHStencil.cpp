...
void TurbFGHStencil::apply ( TurbFlowField & flowField,  int i, int j ){
    ...
    loadLocalTurbViscosity2D(  flowField, _parameters, _localTurbViscosity, i, j);
    ...
    values [0] = computeF2D(_localVelocity, _localMeshsize, _localTurbViscosity, _parameters, _parameters.timestep.dt);
    values [1] = computeG2D(_localVelocity, _localMeshsize, _localTurbViscosity, _parameters, _parameters.timestep.dt);
...
void TurbFGHStencil::apply ( TurbFlowField & flowField, int i, int j, int k ){
    ...
    if ((obstacle & OBSTACLE_SELF) == 0){   // If the cell is fluid
        ...
        loadLocalTurbViscosity3D(  flowField, _parameters, _localTurbViscosity, i, j, k);
        loadLocalMeshsize3D(_parameters, _localMeshsize, i, j, k);

        if ((obstacle & OBSTACLE_RIGHT) == 0) { // If the right cell is fluid
            values [0] = computeF3D(_localVelocity, _localMeshsize, _localTurbViscosity, _parameters, _parameters.timestep.dt);
        }
        if ((obstacle & OBSTACLE_TOP) == 0) {
            values [1] = computeG3D(_localVelocity, _localMeshsize, _localTurbViscosity, _parameters, _parameters.timestep.dt);
        }
        if ((obstacle & OBSTACLE_BACK) == 0) {
            values [2] = computeH3D(_localVelocity, _localMeshsize, _localTurbViscosity, _parameters, _parameters.timestep.dt);
        }
    }
}
