 ...
 */
void TurbVTKStencil::apply ( TurbFlowField & flowField, int i, int j ) {
    ...
    if ((flowField.getFlags().getValue(i,j) & OBSTACLE_SELF) == 0) {
        ...
        FLOAT turbViscosity;
        FLOAT velocity[2];
        flowField.getPressureVelocityAndTurbVisc(pressure, turbViscosity, velocity, i, j);
 ...
 */
void TurbVTKStencil::apply ( TurbFlowField & flowField, int i, int j, int k ) {
    ...
    if ((flowField.getFlags().getValue(i,j,k) & OBSTACLE_SELF) == 0) {
        ...
        FLOAT turbViscosity;
        FLOAT velocity[3];
        flowField.getPressureVelocityAndTurbVisc(pressure, turbViscosity, velocity, i, j, k);
        _pressureStream << pressure << std::endl;
        _turbViscosityStream << turbViscosity << std::endl;
 ...
 */
void TurbVTKStencil::writeHeaderAndCoords() {
    // This is the header
 ...
 */
bool TurbVTKStencil::openFile( int timeStep ) {
    // fileName stores the file name as per XML file
    // it adds current timestep + .vtk extension to it
    std::stringstream fileName;
    fileName << _parameters.vtk.prefix << "_rank_"<< _parameters.parallel.rank << '_' << timeStep << '_' << ".vtk";
 ...
 */
void TurbVTKStencil::write ( TurbFlowField & flowField, int timeStep ) {
