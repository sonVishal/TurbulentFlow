#include "VTKMPIStencil.h"

/** Constructor
 *
 * @param prefix String with the prefix of the name of the VTK files
 */
VTKMPIStencil::VTKMPIStencil ( const Parameters & parameters ) : FieldStencil<FlowField> ( parameters ) {
 ...
 */
void VTKMPIStencil::apply ( FlowField & flowField, int i, int j ) {
 ...
 */
void VTKMPIStencil::apply ( FlowField & flowField, int i, int j, int k ) {
 ...
 */
void VTKMPIStencil::writeHeaderAndCoords() {
    // This is the header
    ...
    if (_parameters.geometry.dim == 2) {
        for (int j = 0; j < _localSize[1]+4; j++) {
            for (int i = 0; i < _localSize[0]+4; i++) {
                ...
                _outputFile << ' ' << _parameters.meshsize->getPosY(i,j) << ' ' << z << std::endl;
            }
        }
    } else {
 ...
 */
bool VTKMPIStencil::openFile( int timeStep ) {
	int rank;
	MPI_Comm_rank(MPI_COMM_WORLD,&rank);
    // fileName stores the file name as per XML file
    // it adds current timestep + .vtk extension to it
    std::stringstream fileName;
    fileName << _parameters.vtk.prefix << "_All"<<'_'<<"rank_"<<rank<<"_" << timeStep << ".vtk";
 ...
 */
void VTKMPIStencil::write ( FlowField & flowField, int timeStep ) {
